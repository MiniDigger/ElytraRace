...
public class ElytraRaceCommands {
    ...
    public void elytrarace(CommandArgs args) throws CloneNotSupportedException {
        ElytraRaceMain.PREFIX.clone().then("===== ElytraRace =====").style(ChatColor.BOLD).color(ChatColor.GOLD).send(args.getSender());
        ...
        if (args.getSender().hasPermission("elytrarace.admin")) {
            ...
            ElytraRaceMain.PREFIX.clone().then("Use ").then("'/er create <race> <type>'").color(ChatColor.YELLOW).then(" to create a new race!")
                    .send(args.getSender());
    ...
    public void portal(CommandArgs args) throws CloneNotSupportedException {
        ...
        ElytraRacePortal p = new ElytraRacePortal(race, type, no);

        Location loc = args.getPlayer().getTargetBlock((Set<Material>) null, 10).getLocation();
        int size = p.createPortal(loc);
        if (size > 0) {
            ...
            ElytraRaceMain.PREFIX.clone().then("Portal creation failed! Are you at a portal structure?").color(ChatColor.RED).send(args.getSender());
        }
    }
